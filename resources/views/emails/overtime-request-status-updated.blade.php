...
<head>
    ...
    <title>Overtime request update</title>
</head>
                    ...
                    <tr>
                        <td style="padding:38px;">
                            ...
                            <p style="margin:0; color:#475569; font-size:16px; line-height:1.7;">
                                Your overtime on <strong style="color:#0f172a;">{{ $overtimeRequest->work_date->format('M d, Y') }}</strong> was {{ $summary }}.
                            </p>

                            @if ($approved && (float) $overtimeRequest->hours_approved < (float) $overtimeRequest->hours_requested)
                                    ...
                                    <tr>
                                        <td style="padding:18px 20px;">
                                            <p style="margin:0; color:#9a3412; font-size:14px; line-height:1.6;">
                                                You filed {{ rtrim(rtrim(number_format((float) $overtimeRequest->hours_requested, 2), '0'), '.') }} hour(s)
                                                and {{ rtrim(rtrim(number_format((float) $overtimeRequest->hours_approved, 2), '0'), '.') }} hour(s) were approved.
                                            </p>
                                        </td>
                                    </tr>
                                </table>
                            ...
                            @if ($overtimeRequest->manager_note)
                                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:24px 0 0; border-radius:14px; background:#f8fafc; border:1px solid #e2e8f0;">
                                    <tr>
                                        <td style="padding:18px 20px;">
                                            <p style="margin:0; color:#475569; font-size:14px; line-height:1.6;"><strong style="color:#0f172a;">Note from your manager:</strong> {{ $overtimeRequest->manager_note }}</p>
                                        </td>
                                    </tr>
                                </table>
                            @endif
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0" style="margin:30px 0;">
                                <tr>
                                    <td style="border-radius:12px; background:#157a52;">
                                        <a href="{{ $url }}" style="display:inline-block; padding:14px 24px; color:#ffffff; font-size:15px; font-weight:700; text-decoration:none; border-radius:12px;">View Request</a>
                                    </td>
                                </tr>
                            </table>
