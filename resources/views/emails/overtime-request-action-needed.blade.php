@php
    $employeeName = $overtimeRequest->employee->fullName() ?: $overtimeRequest->employee->employee_id;
...
<head>
    ...
    <title>Overtime request awaiting approval</title>
</head>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px; overflow:hidden; border-radius:18px; background:#ffffff; border:1px solid #dbe3ee;">
                    <tr>
                        <td style="padding:34px 38px; background:linear-gradient(135deg,#020617 0%,#0f172a 52%,#052e23 100%);">
                            ...
                            <h1 style="margin:0; color:#ffffff; font-size:28px; line-height:1.25; font-weight:800;">Overtime needs approval</h1>
                            <p style="margin:12px 0 0; color:#cbd5e1; font-size:15px; line-height:1.7;">A team member filed overtime and is waiting for your review.</p>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:38px;">
                            ...
                            <p style="margin:0; color:#475569; font-size:16px; line-height:1.7;">
                                <strong style="color:#0f172a;">{{ $employeeName }}</strong> filed
                                {{ rtrim(rtrim(number_format((float) $overtimeRequest->hours_requested, 2), '0'), '.') }} hour(s)
                                of overtime on {{ $overtimeRequest->work_date->format('M d, Y') }}.
                            </p>
                                ...
                                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:24px 0 0; border-radius:14px; background:#f8fafc; border:1px solid #e2e8f0;">
                                    <tr>
                                        <td style="padding:18px 20px;">
                                            <p style="margin:0; color:#475569; font-size:14px; line-height:1.6;"><strong style="color:#0f172a;">Reason:</strong> {{ $overtimeRequest->reason }}</p>
                                        </td>
                                    </tr>
                                </table>
                            @endif
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:24px 0 0; border-radius:14px; background:#fff7ed; border:1px solid #fed7aa;">
                                <tr>
                                    <td style="padding:18px 20px;">
                                        <p style="margin:0; color:#9a3412; font-size:14px; line-height:1.6;">You can approve fewer hours than filed if attendance shows less.</p>
                                    </td>
                                </tr>
                            </table>
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0" style="margin:30px 0;">
                                <tr>
                                    <td style="border-radius:12px; background:#157a52;">
                                        <a href="{{ $url }}" style="display:inline-block; padding:14px 24px; color:#ffffff; font-size:15px; font-weight:700; text-decoration:none; border-radius:12px;">Review Overtime</a>
                                    </td>
                                </tr>
                            </table>
