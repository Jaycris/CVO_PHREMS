...
/**
 * One page for everything an employee has to ask permission for.
 *
 * Filing and deciding live together, the same way overtime works: whether an
 * approval queue appears depends on whether anyone reports to you, which is a
 * fact about the org chart rather than a permission.
 *
 * The kind of request is a row in request_types, so a new kind appears here the
 * moment HR adds it.
 */
new #[Layout('layouts.app')] class extends Component
{
    ...
    {
        $this->reset(['dates', 'newDate', 'rangeEnd', 'details']);
        $this->typeId = RequestType::active()->ordered()->value('id');
    ...
    /** Days from a previous type would otherwise linger on one that has none. */
    ...
    public function selectedType(): ?RequestType
    {
        return $this->typeId ? RequestType::find($this->typeId) : null;
    }

    /** Adds one day, or every working day in a range when an end date is given. */
    public function addDate(): void
    {
        ...
        for ($day = $start->copy(); $day->lte($end); $day->addDay()) {
            ...
            // shorthand for "the working days between these two" — but a single
            // date is taken at face value, because a Saturday shift is
    ...
    public function submit(EmployeeRequestService $service): void
    {
        $this->errorMessage = null;
        ...
        $this->validate([
            ...
            'typeId.required' => 'Pick what kind of request this is.',
            'details.required' => 'Say what you need and why.',
        ]);

        try {
            $service->submit(
                Auth::user()->employee,
                RequestType::findOrFail($this->typeId),
                $this->details,
                $this->dates,
            );
        } catch (\Throwable $e) {
    ...
    public function withdraw(int $id, EmployeeRequestService $service): void
    {
        $service->cancel(EmployeeRequest::findOrFail($id), Auth::user()->employee);
        $this->statusMessage = 'Request withdrawn.';
    }

    public function review(int $id): void
    {
        $this->decidingId = EmployeeRequest::pending()->findOrFail($id)->id;
        $this->decisionNote = '';
        $this->resetValidation();
        $this->errorMessage = null;
    ...
    public function decide(bool $approved, EmployeeRequestService $service): void
    {
        $this->errorMessage = null;
        ...
        try {
            $service->decide(
                EmployeeRequest::with(['employee', 'type', 'days'])->findOrFail($this->decidingId),
                Auth::user()->employee,
                $approved,
    ...
    public function with(EmployeeRequestService $service): array
    {
        $user = Auth::user();
        ...
        $deciding = $this->decidingId
            ? EmployeeRequest::with(['employee', 'type', 'days'])->find($this->decidingId)
            : null;

        return [
            'types' => RequestType::active()->ordered()->get(),
            'chosenType' => $this->selectedType(),
            'queue' => $queue,
            'mine' => $employee
                ? EmployeeRequest::with(['type', 'days'])->where('employee_id', $employee->id)->latest()
...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold text-[#0f172a] dark:text-white">Requests</h1>
            <p class="text-sm font-medium text-[#778599] dark:text-neutral-400">
                Work from home, a shift change, a certificate, equipment — file it here and your manager decides it.
            </p>
        </div>

        @if ($hasEmployee && $types->isNotEmpty())
            <x-button wire:click="createRequest" @click="$wire.showForm = true" pill>
                <x-icon name="plus" class="h-4 w-4" /> File a Request
            </x-button>
        @endif
    </div>
    ...
    @if ($types->isEmpty())
        <x-card>
            <p class="py-6 text-center text-sm font-medium text-[#778599]">
                No request types are set up yet.
                @can('requests.types.manage')
                    Add them on <a href="{{ route('request-types.index') }}" wire:navigate class="font-semibold text-brand-700 hover:text-brand-800 dark:text-brand-400">Request Types</a>.
                @else
                    Ask HR to add them.
                @endcan
            </p>
                ...
                <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800">
                    <thead class="bg-[#f8fafc] dark:bg-neutral-800/50">
                        <tr>
                            ...
                            <th class="px-4 py-4 text-left text-xs font-medium uppercase tracking-wide text-[#778599]">Request</th>
                            <th class="px-4 py-4 text-left text-xs font-medium uppercase tracking-wide text-[#778599]">Days</th>
                            <th class="px-4 py-4 text-left text-xs font-medium uppercase tracking-wide text-[#778599]">Details</th>
                    ...
                    <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                        @foreach ($queue as $request)
                            <tr wire:key="q-{{ $request->id }}">
                                ...
                                <td class="px-4 py-3"><x-badge color="brand">{{ $request->typeName() }}</x-badge></td>
                                ...
                                <td class="px-4 py-3 font-medium text-[#778599]">{{ $request->details }}</td>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800">
                    ...
                    <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                        ...
                        @empty
                            <tr><td colspan="5" class="px-4 py-10 text-center font-medium text-[#778599]">Nothing filed yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <x-modal wire="showForm" onClose="$set('showForm', false)" maxWidth="lg">
        <h2 class="text-lg font-bold text-[#0f172a] dark:text-white">File a Request</h2>
        ...
        <div class="mt-5 space-y-4">
            <div>
                <x-label>What kind of request?</x-label>
                <x-select wire:model.live="typeId">
                    @foreach ($types as $type)
                        <option value="{{ $type->id }}">{{ $type->name }}</option>
                    @endforeach
                </x-select>
                @if ($chosenType?->description)
                    <p class="mt-1 text-xs font-medium text-[#778599]">{{ $chosenType->description }}</p>
                @endif
                @error('typeId') <p class="mt-1 text-xs font-semibold text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>

            {{-- The date picker only appears for types that are about days. --}}
            @if ($chosenType?->needs_dates)
                <div class="grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
                    ...
                    <x-button wire:click="addDate" type="button" variant="secondary" class="h-11">Add</x-button>
                </div>
            ...
            <div>
                ...
                <x-textarea wire:model="details" rows="3"
                            placeholder="{{ $chosenType?->instructions ?: 'Say what you need and why.' }}" />
                @if ($chosenType?->instructions)
                    <p class="mt-1 text-xs font-medium text-[#778599]">{{ $chosenType->instructions }}</p>
                @endif
                @error('details') <p class="mt-1 text-xs font-semibold text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>
        </div>
    ...
    <x-modal wire="showDecision" onClose="closeDecision" maxWidth="lg">
        @if ($deciding)
            <div class="flex flex-wrap items-start justify-between gap-2">
                ...
                <x-badge color="brand">{{ $deciding->typeName() }}</x-badge>
            </div>
            ...
            <div class="mt-4 rounded-xl bg-[#f8fafc] p-4 dark:bg-neutral-800/50">
                <p class="text-xs font-bold uppercase tracking-wide text-[#778599]">Details</p>
                <p class="mt-1 text-sm font-medium text-[#65758c] dark:text-neutral-300">{{ $deciding->details }}</p>
            </div>

            @if ($deciding->days->isNotEmpty())
                {{-- How many people are already approved for the same thing on
                     each day. Rarely a question about one person. --}}
                <div class="mt-4">
                        ...
                        @foreach ($deciding->days as $day)
                            ...
                            <li class="flex items-center justify-between gap-3 py-2" wire:key="cov-{{ $day->id }}">
                                ...
                                <span class="text-xs font-medium {{ $others > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-[#778599]' }}">
                                    {{ $others === 0 ? 'nobody else' : $others . ' other(s) already approved' }}
                                </span>
                            </li>
                        @endforeach
                    </ul>
