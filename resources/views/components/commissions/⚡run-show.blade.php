...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function with(CommissionSlipNotifier $notifier): array
    {
        $run = $this->run();
...
<div class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            ...
            <h1 class="mt-1 text-xl font-bold text-[#0f172a] dark:text-white">Commissions — {{ $run->monthLabel() }}</h1>
            <p class="text-sm font-medium text-[#778599] dark:text-neutral-400">
                Every figure comes from the CRM. This app copies them down and changes none of them.
            </p>
        </div>
    ...
    <x-card>
        ...
        @if ($run->status === 'draft')
            <p class="mt-3 text-xs font-medium text-[#778599]">
                Nothing has been read yet. Compute reads every agent's figures from the CRM and writes them down.
            </p>
