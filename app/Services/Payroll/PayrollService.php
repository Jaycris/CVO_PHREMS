...
class PayrollService
{
    ...
    protected function payslipAttributes(Employee $employee, array $counters, array $figures): array
    {
        return [
            ...
            // Left at zero when paying per day, so the payslip does not show
            // hours that played no part in the figure.
            'night_diff_hours' => PayslipCalculator::nightDiffPerHour()
                ? round(($counters['night_diff_minutes'] ?? 0) / 60, 2)
                : 0,
    ...
    protected function writeLines(Payslip $payslip): void
    {
        ...
        $add('earning', 'Night differential', (float) $payslip->night_differential_pay,
            (float) $payslip->night_diff_hours > 0
